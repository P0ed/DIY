...
def thread(radius: float, pitch: float, height: float, d, radius_eps, aspect = 10, N = 100, deg = 3, smooth = None, tol = 1e-6):
	e1_bottom = (Workplane("XY")
		.parametricCurve(helix(radius, 0, pitch, height, -d), N = N, minDeg = deg, maxDeg = deg, smoothing = smooth, tol = tol).val()
	)
	e1_top = (Workplane("XY")
		.parametricCurve(helix(radius, 0, pitch, height, d), N = N,  minDeg = deg, maxDeg = deg, smoothing = smooth, tol = tol).val()
    )
	e2_bottom = (Workplane("XY")
		.parametricCurve(helix(radius, radius_eps, pitch, height, -d / aspect), N = N, minDeg = deg, maxDeg = deg, smoothing = smooth, tol = tol).val()
	)
	e2_top = (Workplane("XY")
		.parametricCurve(helix(radius, radius_eps, pitch, height, d / aspect), N = N, minDeg = deg, maxDeg = deg, smoothing = smooth, tol = tol).val()
	)
