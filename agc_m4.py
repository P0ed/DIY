...
def brick(side: float = 1.0) -> Workplane:
    return dif([
        ...
        mirror("YZ") (
            box_fc(wt2 * 2, h - col * 2, t2 + 4.0, '|X', 2.0, c2)
            .translate((w / 2, 0, -wtt - 2.0 if side != 1 else 0))
        ),
    ])
...
def makeTop(ptn: Callable[[int, int], bool]) -> Workplane:
    ...
    return com2(mov(0, 0, t3), dif) ([
        ...
        com2(mirror("YZ"), mov(inch, 1.5 * inch, t3)) (
            box_fc(2 * inch - wtt / 2, 3 * inch - wtt / 2, wt, '|Z', 2.0, c2)
        ),
        mov(0, -1.5 * inch, t3) (
            box_fc(4 * inch - wtt / 2, 3 * inch - wtt / 2, wt, '|Z', 2.0, c2)
        ),
    ])
...
comp = bot + top.translate((0, 0, t2 + 0.015))
show(comp)
